...
def mask(row):
	if row['Клиент'] == 'Общество с ограниченной ответственностью «Румянцево»':
		return 'Общество с ограниченной ответственностью «Румянцево»'
	if row['Клиент'] == 'ООО "ДИСКОНТ"':
		return 'ООО "ДИСКОНТ"'
	if row['Клиент'] == 'ООО "МЭ"':
		return 'ООО "МЭ"'
	else:
		return None

df['Клиент'] = df.loc[:, ['Клиент']].apply(mask, axis=1)
df.dropna(subset=['Клиент'], how='any', axis=0, inplace=True)

# df = df[df['Клиент'] == ООО "ДИСКОНТ"]
